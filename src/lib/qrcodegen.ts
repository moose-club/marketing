 ...
 *
 * Vendored from Project Nayuki's reference implementation (MIT License):
 *   https://www.nayuki.io/page/qr-code-generator-library
 *
 * Self-contained, zero runtime dependencies. Only imported by the /claim
 * page's client script, so it adds no weight to the rest of the site
 * (which still ships zero JS). We vendor rather than add an npm dependency
 * to keep the build hermetic — see CLAUDE.md "no UI kit" discipline.
 *
 * Only the byte-mode encoding path and SVG output are used here; the rest of
 * the library is kept intact so it behaves exactly like upstream.
 */
